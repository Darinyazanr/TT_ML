...
MODELPATH='/home/jq/jeeker/Model_2.1/RF'
...
def normalX(trainFile):
    ...
    pickle.dump(max_abs_scaler, open(MODELPATH+'/MaxAbsScaler.pickle', 'wb'), -1)
...
def testModel(testFile):
    import json
    import pickle
    model=pickle.load(open(MODELPATH+'/RF_zhuyuan.pickle','rb'))
    import time
    start=time.time()
    from sklearn.datasets import load_svmlight_file
